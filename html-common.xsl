...
<xsl:stylesheet
  ...
  version="1.0">
	...
	<xsl:template match="titlestmt">
        ...
        <xsl:if test="titleproper">
            <tr>
                <td><span class="fieldname">Title</span></td>
                <td><xsl:apply-templates select="titleproper"/></td>    
            </tr>
        </xsl:if>
        <xsl:if test="subtitle">
            <tr>
                <td><span class="fieldname">Sub-title</span></td>
                <td><xsl:apply-templates select="subtitle"/></td>    
            </tr>
        </xsl:if>
        <xsl:for-each select="author">
            <tr>
                <td><span class="fieldname">Author</span></td>
                <td><xsl:apply-templates select="."/></td>    
            </tr>
        </xsl:for-each>
        <xsl:if test="sponsor">
            <tr>
                <td><span class="fieldname">Sponsor</span></td>
	...
	<xsl:template match="profiledesc">
        <xsl:if test="creation">
            <tr>
                <td><span class="fieldname">Creation</span></td>
                <td><xsl:apply-templates select="./creation"/></td>    
            </tr>
        </xsl:if>
        <xsl:if test="descrules">
            <tr>
                <td><span class="fieldname">Descriptive Rules</span></td>
                <td><xsl:apply-templates select="./descrules"/></td>    
            </tr>
        </xsl:if>
        <xsl:if test="langusage">
            <tr>
                <td><span class="fieldname">Language Usage</span></td>
	...
	<xsl:template match="revisiondesc">      
        <xsl:if test="./text()">
            <tr>
                <td><span class="fieldname">Revisions</span></td>
            ...
            <xsl:variable name="headstring"><xsl:text>Administrative / Biographical History</xsl:text></xsl:variable>
		 	<xsl:choose>
		 		<xsl:when test="../../archdesc or ../../../c3:component">
		 			<h2 class="fieldhead-auto"><xsl:value-of select="$headstring"/></h2>
		 		</xsl:when>
				<xsl:otherwise>
					<h3 class="fieldhead-auto"><xsl:value-of select="$headstring"/></h3>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
		<xsl:apply-templates/>
  ...
  <xsl:template match="separatedmaterial">
	...
	<xsl:template match="head">
		<xsl:choose>
			<xsl:when test="../../head or not(../../../archdesc or ../../../../c3:component)">
				<h3 class="fieldhead-ead"><xsl:apply-templates /></h3>
			</xsl:when>
			<xsl:otherwise>
				<h2 class="fieldhead-ead"><xsl:apply-templates /></h2>				
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
    ...
    <xsl:template match="note" mode="own-section">
        <xsl:if test="not(head/text())">
            <xsl:variable name="headstring"><xsl:text>Note</xsl:text></xsl:variable>
            <xsl:choose>
                <xsl:when test="../../archdesc or ../../../c3:component">
                    <h2 class="ead"><xsl:value-of select="$headstring"/></h2>
                </xsl:when>
                <xsl:otherwise>
...
{ 
	...
	<xsl:template name="cgiencode">
	    <xsl:param name="text"/>
	    <!-- following line contains a weird invisible character propably a Windows \r\n or something! -->
