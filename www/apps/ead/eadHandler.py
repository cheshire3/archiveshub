...
# Description:
#            Globals and parent class web interfaces to a Cheshire3 database of
#            EAD finding aids. Part of Cheshire for Archives v3
#
...
#
# Version History:
# 0.01 - 12/10/2007 - JH - Globals migrated from separate files, parent class
#                          created
# 0.02 - 06/11/2007 - JH - Migrated to new architecture:
#                          extractor --> tokenizer -- tokenMerge
...
# 0.05 - 03/01/2008 - CS - _parse_upload function moved here from adminHandler
#                          as also used in editing handler
# 0.06 - 12/02/2008 - CS - _walk_directory function moved here from
#                          adminHandler as also used in editing handler
# 0.07 - 09/01/2008 - JH - _parse_upload bug fix
# 0.08 - 22/11/2010 - JH - Provision for exception logging / reporting
# 0.09 - 15/03/2011 - JH - Bug fixe for ToC character encoding fixed - coerce
#                          to unicode when read in
...
class EadHandler(object):
    # Hierarchical class - must be subclassed to actually interact with
    # database
    logger = None
    excLogger = None
    htmlTitle = []
    ...
    def _stripOffendingChar(self, exception):
        ...
        return (text[:exception.start] +
                '<span class="error" title="This character could not be ' +
                'encoded for display">*</span>' +
                text[exception.end:])
            ...
            try:
                ...
                try:
                    ...
                    if interface == 'admin':
                        ...
                        link = ('<a href="edit.html">'
                                'Back to edit/create menu</a>')
                    return ('<div id="single">'
                            '<p class="error">An error occured while parsing '
                            'your file. Please check the file is a valid ead '
                            'file and try again.</p><p>%s</p>'
                            '</div>' % link)
                else:
                    if interface == 'admin':
                        link = '<a href="files.html">Back to file page</a>'
                    ...
                    return ('<div id="single">'
                            '<p class="error">An error occured while parsing '
                            'your file. Please check the file at the '
                            'suggested location and try again.</p>'
                            '<code>%s: %s</code>'
                            '<pre>%s<span class="error">%s%s^</span></pre>'
                            '<p>%s</p>'
                            '</div>' % (html_encode(repr(e[0])),
                                        e[1],
                                        html_encode(line[:posn + 20]) + '...',
                                        startspace,
                                        str('-' * (posn - len(startspace))),
                                        link)
                            )
        ...
        for f in filelist:
            if (os.path.isdir(os.path.join(d, f))):
                ...
                if link:
                    outF.append('<li>'
                                '<span class="fileops">'
                                '<input type="%s" name="filepath" value="%s"/>' 
                                '</span>'
                                '<span class="filename"><a href="files.html?'
                                'operation=view&amp;filepath=%s" '
                                'title="View file contents">%s</a></span>'
                                '</li>' (type, fp, cgi_encode(fp), f)
                                )
                else:
                    outF.append('<li>'
                                '<span class="fileops">'
                                '<input type="%s" name="filepath" '
                                'value="%s"/></span>'
    ...
    def display_full(self, rec, paramDict, pageNavType='form'):
        ...
        if (l < max_page_size_bytes):
            ...
            write_file(os.path.join(cache_path, recid + '-p1.shtml'),
                       pages[0].encode('utf-8'))
        else:
            ...
            while pseudopages:
                pagebits = ['<div id="leftcol" class="toc">'
                            ...
                            'unavailable ]" -->',
                            '<!--#include virtual="/ead/tocs/RECID.inc"-->',
                            '</div>',
                ...
                # Append: pagenav, end rightcol div, padder div,
                # left div (containing toc)
            ...
            self.logger.log('Links resolved over multiple pages (%d pages)'
                            '' % (len(pages)))

            for x in range(len(pages)):
                doc = pages[x]
                # Now we know how many real pages there are, generate some page
                # navigation links
                pagenav = []
                if len(pages) > 1:
                    ...
                    if (x > 0):
                        pagenav.extend(['<a href="%s/%s-p1.shtml" '
                                        'title="First page" '
                                        'onclick="setCookie(\'%s-tocstate\', '
                                        'stateToString(\'someId\'))">%s</a>'
                                        '' % (cache_url,
                                              recid,
                                              recid,
                                              fback_tag), 
                    ...
                    if pageNavType == 'form':
                        ...
                        pagenav.extend(
                            ['<form action="%s">Page: ' % (self.script),
                             '<input type="hidden" name="operation" '
                             'value="full" />',
                             '<input type="hidden" name="recid" value="%s" />'
                             '' % (recid),
                             '<input type="text" name="page" size="2" '
                             'maxlength="3" value="%d"/> of %d'
                             '' % (x + 1, len(pages)),
                        ...
                        for y in range(len(pages)):
                            if (y == x):
                                ...
                                pagenav.append('<a href="%s/%s-p%d.shtml" '
                                               'title="Page %d" '
                                               'onclick="setCookie(\'%s-'
                                               'tocstate\', '
                ...
                write_file(os.path.join(cache_path,
                                        recid + '-p%d.shtml' % (x + 1)),
                           pagex)
...
def build_architecture(data=None):
    ...
    global fullTxr, fullSplitTxr, ppFlow
    global rebuild
...
# Following busts when there are accented chars(!?)
#punctuationRe = re.compile('([@+=;!?:*"{}()\[\]\~/\\|\#\&\^]|[-.,\']'
#                           '(?=\s+)|(?<=\s)[-.,\'])')
wordRe = re.compile('\s*\S+')
# Acceptable E-mail address, e.g. foo@bar.com
emailRe = re.compile('^[0-9a-zA-Z][^@ .]*(\.[^@ .]+)*'
                     '@[^@ .]+\.[^@ .]+(\.[^@ .]+)*$')
anchorRe = re.compile('<a .*?name="(.*?)".*?>')
overescapedAmpRe = re.compile('&amp;([^\s]*?);')
nonAsciiRe = re.compile('([\x7b-\xff])')
# First date and time stamp in file
firstlog_re = re.compile('^\[(\d\d\d\d\-\d\d\-\d\d \d\d:\d\d:\d\d)\]',
                         re.MULTILINE)
