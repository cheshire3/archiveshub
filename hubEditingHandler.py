...
#            Py: hubLocalConfig.py
#            HTML: ead2002.html, edithelp.html, editmenu.html, header.html, footer.html, template.html
...
# 0.04 - 07/06/2011 - JH - Prevent cgitb module from hiding server errors from AJAX
#                        - Bug fix for punctuation array single-item tuples (foo,) rather than (foo)
# 0.05 - 17/11/2011 - JH - Sort users alphabetically, case insensitive, regardless of underlying dbms (SQL) 
...
class HubEditingHandler(object):
    ...
    def build_ead(self, form, template=False):
        ...
        if template == True:
            ...
            for field in mylist:
                if field.name not in ['ctype','location','operation', 
        ...
        if (loc == 'collectionLevel'):
            ...
            self._add_text(target, 'Created by {0} using the Archives Hub EAD Editor'.format(userName))
        ...
        if (collection):
            ...
            valid = self._updateNodeFromFieldList(node, mylist, component=True)
...
# Navigation related functions ==================================================================
    ...
    def navigate(self, form): 
        ...
        if self._hasPermission('%s-%s' % (recid, owner)):
            ...
            return '<p>You do not have permission to perform the requested operation on the requested record</p>'
    ...
    def populate_form(self, recid, owner, new, form):  
        ...
        page = page.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" disabled="disabled" value="%s"/>' % recid)
    ...
    def save_template(self, form):
        ...
        sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % session.user.username
        res = authStore._query(sqlQ)
        ...
        templateStore.unlink(session, 'linktempinst', rec, institutionid=institutionid)
        templateStore.link(session, 'linktempinst', rec, institutionid=institutionid)
    ...
    def discard_template(self, form):
        ...
        if not recid == None:
            try:
                templateStore.delete_record(session, '%s-%s' % (recid, institutionid))
                templateStore.commit_storing(session)
                return 'done'   
            except:
                return 'failed'
    ...
    def generate_file(self, form):
        ...
        if (quota - total) > 0:  
            ...
            page = page.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" readonly="true" class="readonly"/>')
    ...
    def load_file(self, form, req):        
        ...
        if found and self._hasPermission(recid) :
            ...
            if splitId[-1] == session.user.username:
                page = page.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" disabled="disabled" value="%s"/>' % '-'.join(splitId[:-1]))
            else :
                page = page.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" disabled="disabled" value="%s"/><input type="hidden" id="owner" value="%s"/>' % ('-'.join(splitId[:-1]), splitId[-1]))                             
            page = page.replace('%TOC%', tocTxr.process_record(session, rec).get_raw(session))
            page = page.replace('%MENUCODE%', read_file('contextMenu.html'))
            page = page.replace('%KYBDCODE%', read_file('keyboard.html'))
            return page
        else:
            return '<p>You do not have permission to perform the action requested on the record requested</p>'
    ...
    def upload_local(self, form):
        ...
        if remaining_quota < 1:
            ...
            if not rec.process_xpath(session, '/ead/eadheader'): 
                ...
                if idCheck[0] == True:          
                    if idCheck[2] == True:
                        return '''<p>You already have this file open for editing as {0}.</p>
                                  <p><a href="edit.html?operation=load&user=null&recid={0}-{1}">click here</a> to continue editing the version of the file in the Draft File Store.
                                  If you reached this page by using the back button from a preview of the record then this will take you back to the record you were editing.<p>
                                  <p>To edit the version from your local file store please delete the file currently in the Draft File Store before reloading</p>
                                  <a href="menu.html">Back to Create/Edit Menu</a>'''.format(recid, un.encode('ascii', 'ignore'))
                        
                id = '{0}-{1}'.format(recid, un.encode('ascii', 'ignore'))
                rec1.id = id
                ...
                try:          
                    ...
                    # Get the institution of the user performing the operation
                    sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='{0}'".format(un)
                    res = authStore._query(sqlQ)
                    result = res.dictresult()
                    if len(result) > 1:
                        ...
                        institutionid = result[0]['institutionid']
                    # Otherwise store in editing store and create institution link
                    editStore.store_record(session, rec1)
                    editStore.link(session, 'linkrecinst', rec1, institutionid=institutionid)
                ...
                if not self._hasPermission(rec1.id): 
                    ...
                    page = page.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" disabled="true" value="%s"/>' % (recid.encode('ascii')))
    ...
    def _hasPermission(self, recid):
        """Confirm or deny current user has permission to edit requested record."""
        # Get the institution of the user performing the operation
        sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % session.user.username
        res = authStore._query(sqlQ)
        result = res.dictresult()
        ...
        sqlQ = "SELECT institutionid FROM editingstore_linkrecinst WHERE editingstore='%s' LIMIT 1" % recid
        res = editStore._query(sqlQ)
        result = res.dictresult()
        if len(result) == 0:
            return False
        elif len(result) > 1:
    ...
    def _hasTemplatePermission(self, recid):
        """Confirm or deny current user has permission to edit requested template."""
        # Get the institution of the user performing the operation
        templateStore = db.get_object(session, 'templateStore')
        sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % session.user.username
        res = authStore._query(sqlQ)
        ...
        if len(result) == 0:
            ...
            authinst = result[0]['institutionid']
        sqlQ = "SELECT institutionid FROM templatestore_linktempinst WHERE templatestore='%s'" % recid
        res = templateStore._query(sqlQ)
        result = res.dictresult()
        if len(result) == 0:
            return False
        elif len(result) > 1:
...
<response>
    ...
    def save_form(self, form):
        ...
        if (loc == 'collectionLevel' and 
            ...
            editStore.link(session, 'linkrecinst', rec, institutionid=institutionid)
#            editStore.commit_storing(session) 
            ...
            # Don't simply get_dom in case recordStore does not return LxmlRecords
            ...
            # Add/delete sponsor
            if (form.get('filedesc/titlestmt/sponsor', '').value.strip() != '' and 
                form.get('filedesc/titlestmt/sponsor', '').value.strip() != ' '):
                target = self._create_path(header, 'filedesc/titlestmt/sponsor')
                self._add_text(target, form.get('filedesc/titlestmt/sponsor', ''))
            else:
            ...
            if self._hasPermission(rec.id):
                ...
                return ('<p>You do not have permission to perform the action requested on the record requested</p>', False)
#            editStore.commit_storing(session)
...
#                editStore.commit_storing(session)
    ...
    def _updateNodeFromFieldList(self, node, fieldlist, component=False):
        ...
        deleteList = []
        daonames = {}
        self.log(",".join(["{0.name}={0.value}".format(f) for f in fieldlist]))
        for field in fieldlist:
            if field.name not in ['ctype', 'location', 'operation', 
                                      'newForm', 'owner', 'recid', 'parent', 
                                      'pui', 'filedesc/titlestmt/sponsor', 
                                      'daoselect']:
                ...
                if field.name.startswith('controlaccess'):
                    ...
                    if (field.value.strip() != '' and 
                        ...
                        self._add_text(target, field.value)   
                        try:
                            validList.remove(re.sub("\[\d+\]", "", field.name))
                        except:
                            try:
                                validList.remove(field.name)
                            except:
                        ...
                        if (field.name.startswith('did/unitid') and 
                            field.name.endswith(']')):
                            ...
                            if (field.name == 'did/unitid[1]' and not
                                node.xpath('did/unitid[@type="persistent"]')):
                                # Add type="persistent"
                                attr_target = self._create_path(target, "@type")
    ...
    def preview_file(self, form):
        # Adapted from record function in hubSearchHandler (now in hubHandler, JH Oct '11)
    ...
    def display_full(self, rec, paramDict):
        ...
        pages = []
        while pseudopages:
            pagebits = ['<div id="leftcol" class="toc"><!--#config errmsg="[ Table of Contents unavailable ]" --><!--#include virtual="/hubedit/tocs/RECID.inc"--></div>'
        ...
        for x, pagex in enumerate(pages):
            ...
            if len(pages) > 1:
                ...
                if (x > 0):
                    pagenav.extend(['<a href="%s/%s-p1.shtml" title="First page"))">%s</a>' % (cache_url, recid, fback_tag), 
                                    '<a href="%s/%s-p%d.shtml" title="Previous page">%s</a>' % (cache_url, recid, x, back_tag)
                                  ])
                pagenav.extend(['</div>', '<div class="forwardlinks">'])
                if (x < len(pages)-1):
                    pagenav.extend(['<a href="%s/%s-p%d.shtml" title="Next page">%s</a>' % (cache_url, recid, x+2, forward_tag),
                                    '<a href="%s/%s-p%d.shtml" title="Final page">%s</a>' % (cache_url, recid, len(pages), fforward_tag)
                                  ])
            ...
            write_file(os.path.join(cache_path, recid + '-p%d.shtml' % (x+1)), tmpl.replace('%CONTENT%', pagex))
        if tocfile:
            try:
                for k, v in anchorPageHash.iteritems():
    ...
    def preview_xml(self, form):
        ...
        if self._hasPermission('%s-%s' % (recid, fileOwner)):
            if recid != None and recid != 'null' :
                retrievedRec = editStore.fetch_record(session, '%s-%s' % (recid, fileOwner))
                outDoc = db.get_object(session, 'orderingTxr').process_record(session, retrievedRec) 
                if markup == 'interop':
    ...
    def savetodisk(self, form, req):
        ...
        try:          
            files = glob.glob('%s/files/%s*' % (cache_path, session.user.username))
            for f in files :
                os.remove(f)
        except:
            pass
        ...
        if self._hasPermission(rec.id):
            ...
            f = open(os.path.join(cache_path, 'files', '%s.xml' % session.user.username), 'w')
            f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
            f.write(db.get_object(session, 'indentingTxr').process_record(session, rec).get_raw(session))
            f.flush()
            f.close()
            req.headers_out["Content-Disposition"] = "attachment; filename=%s.xml" % recid
            req.content_type = "text/plain"
            try:
                req.sendfile(os.path.join(cache_path, 'files', '%s.xml' % session.user.username))
            except IOError, e:
                req.content_type = "text/html"
            ...
            req.write('You do not have permission to perform the action requested on the record requested')
            return apache.OK
 
    #- end savetodisk() ---------------------------------------------------------
      
    def send_email(self, address=None, recid=None, fileOwner=None, mess=None, markup="hub"):
        if recid != None and recid != 'null' :
            rec = editStore.fetch_record(session, '%s-%s' % (recid, fileOwner))
        ...
        message['From'] = '%s@%s' % (outgoing_email_username, outgoing_email_suffix)
        message['To'] = address
        message['Subject'] = 'EAD record %s' % (recid)
        if mess == None:
            message.attach(MIMEText('The EAD record you requested is attached.'))
        else:
            message.attach(MIMEText(mess))
        part = MIMEBase('application', "octet-stream")
        part.set_payload('<?xml version="1.0" encoding="UTF-8"?>\n%s' % db.get_object(session, 'indentingTxr').process_record(session, rec).get_raw(session))
        encoders.encode_base64(part)
        part.add_header('Content-Disposition', 'attachment; filename="%s.xml"' % recid)
        message.attach(part)
        smtp = smtplib.SMTP()
        smtp.connect(host=outgoing_email_host, port=25)
        smtp.sendmail('%s@%s' % (outgoing_email_username, outgoing_email_suffix), address, message.as_string())
        smtp.close()
        return ('''\
        <div id="single">
    ...
    def emailRec(self, form):
        ...
        if self._hasPermission('%s-%s' % (recid, owner)):
            if not address:          
                repl = ['<input type="hidden" name="recid" value="%s"/>' % recid,
                ...
                return self.send_email(recid=recid, fileOwner=owner, address=address, markup=markup) 
        else:
            return '<p>You do not have permission to perform the requested operation on the requested record</p>' 
    ...
    def emailHub(self, form):
        ...
        if self._hasPermission('%s-%s' % (recid, owner)):
            if not address:          
                ...
                return self.send_email(recid=recid, fileOwner=owner, address=hubemailaddress, mess=message) 
        else:
            return '<p>You do not have permission to perform the requested operation on the requested record</p>' 
    ...
    def add_form(self, form):
        ...
        if self._hasPermission('%s-%s' % (recid, fileOwner)):
            ...
            rec = xmlp.process_document(session, doc)
            htmlform = formTxr.process_record(session, rec).get_raw(session)
            htmlform = htmlform.replace('%PUI%', '<input type="text" onfocus="setCurrent(this);" name="pui" id="pui" size="25" disabled="true" value="%s"/>' % recid)
            return htmlform
        else:
            return '<p>You do not have permission to perform the requested operation on the requested record</p>'
    ...
    def show_userMenu(self, values=None, message='', display=None):
        if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore') and display == None:
    ...
    def list_users(self, values=None, message=''):
        ...
        for ctr, uid in enumerate(self._get_usernamesFromMyInst()):
            ...
            if (uid == session.user.id):
                cells = cells + '<td><a href="users.html?operation=displayedit" class="fileop">EDIT</a></td>'
            elif (session.user.has_flag(session, 'info:srw/operation/1/delete', 'hubAuthStore')):
                cells = cells + '<td><a href="users.html?operation=deleteuser&amp;user=%s&confirm=true" class="fileop">DELETE</a></td>' % (uid)  
            else:
                cells = cells + '<td>N/A</td>'
            lines.append('<tr class="%s">%s</tr>' % (rowclass, cells))
        lines.append('</table><br/>')
        if (session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore')):
    ...
    def edit_user(self, form):
        ...
        try:
            ...
            if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
                return self.show_userMenu(values, '<p class="error">User with id "%s" does not exist!</p>' % userid, 'display')
            else:
                return self.show_userMenu(values, '<p class="error">User with id "%s" does not exist!</p>' % userid)
            ...
            if (passwd and user.check_password(session, passwd)):
                ...
                if (passwd1 and passwd1 != ''):
                    ...
                    if (passwd1 == passwd2):
                        ...
                        if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
                            return self.show_userMenu(values, '<p class="error">Unable to update details - new passwords did not match.</p>', 'display')
                        else:
                            return self.show_userMenu(values, '<p class="error">Unable to update details - new passwords did not match.</p>')
                # Update DOM
                ...
                if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
                    ...
                    return self.show_userMenu(values, '<p class="ok">Details successfully updated.</p>')
            else:
                if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):             
                    return self.show_userMenu(values, '<p class="error">Unable to update details - current password missing or incorrect.</p>', 'display')               
                else:
                    return self.show_userMenu(values, '<p class="error">Unable to update details - current password missing or incorrect.</p>') 
        else:
    ...
    def delete_user(self, form):
        ...
        passwd = form.get('passwd', None)
        if (confirm == 'true'):
            output = ['<div id="maincontent"><h3 class="bar">Delete User Confirmation.</h3>', read_file('deleteuser.html').replace('%USERID%', userid), '</div>']
            return ''.join(output)      
        elif (cancel == 'Cancel'):
            return self.show_userMenu()
        else:
            if (passwd and session.user.check_password(session, passwd)):
                if (self._canDelete(userid) and session.user.has_flag(session, 'info:srw/operation/1/delete', 'hubAuthStore')):
                    try :
                        authStore.delete_record(session, userid)
                    except :
                        return self.show_userMenu(None, '<p class="error">Unable to delete user %s - user does not exist.</p>' % (userid))
                    else :
                        rebuild = True
                        return self.show_userMenu()
                else :
                    return self.show_userMenu(None, '<p class="error">Unable to delete user %s - you do not have permission to perform this operation.</p>' % (userid))
            else :
                return self.show_userMenu(None, '<p class="error">Unable to delete user %s - incorrect password.</p>' % (userid))
    ...
    def _canDelete(self, userid):
        ...
        if len(result) == 0:
            ...
            authinst = result[0]['institutionid']
        sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % userid
        res = editStore._query(sqlQ)
        result = res.dictresult()
        if len(result) == 0:
            return False
    ...
    def add_user(self, form):
        ...
        if (form.get('submit', None)):
            ...
            if not (userid and userid.isalnum()):
                ...
                values['%USERNAME%'] = ''
                return self.show_userMenu(values, '<p class="error">%s</p>' % message)
            try:
                user = authStore.fetch_object(session, userid)
            ...
            if user is not None:
                ...
                userRec = xmlp.process_document(session, StringDocument(userXML))
                userNode = userRec.get_dom(session)
                passwd = form.get('passwd', None)
                # check password
                newHash = {}
                ...
                if (passwd1 and passwd1 != ''):
                    ...
                    if (passwd1 == passwd2):
                        ...
                        return self.show_userMenu(values, '<p class="error">Unable to add user - passwords did not match.</p>')
                else:
                    return self.show_userMenu(values, '<p class="error">Unable to add user - password not supplied.</p>')      
                sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % session.user.id
                res = authStore._query(sqlQ)
                result = res.dictresult()
                ...
                authStore.link(session, 'linkAuthInst', user, institutionId=inst)
                rebuild = True
                values['%USERNAME%'] = ''
                values['%realName%'] = ''
                values['%email%'] = ''
    ...
    def reassign_record(self, form):
        ...
        rec = editStore.fetch_record(session, recid)
        if self._hasPermission(recid) and session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
            if rec.id[rec.id.rfind('-')+1:] == newUser :
                return recid
            else :
                sqlQ = "SELECT institutionid FROM hubauthstore_linkauthinst WHERE hubauthstore='%s'" % newUser
                res = authStore._query(sqlQ)
                result = res.dictresult()
                if len(result) > 1:
                    #we have two users with the same is - should never happen
    ...
    def _get_usernamesByInst(self, institutionid):
        ...
        sqlQ = "SELECT hubauthstore FROM hubauthstore_linkauthinst WHERE institutionid='%s' ORDER BY hubauthstore ASC" % (institutionid)
        res = authStore._query(sqlQ)
    ...
    def _get_templatesByInst(self):
        ...
        result = res.dictresult()
        institutionid = result[0]['institutionid']
        sqlQ = "SELECT templatestore FROM templatestore_linktempinst WHERE institutionid='%s'" % (institutionid)
        res = authStore._query(sqlQ)
        result = res.dictresult()
        return result
    ...
    def show_editMenu(self):
        ...
        if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
            ...
            for ctr, uid in enumerate(self._get_usernamesFromMyInst()):
                ...
                users.append('<option value="{0.username}">{0.username}</option>'.format(user))
            assignmentOptn = '''\
            <select id="userSelect" name="user" disabled="true">
                <option value="null">Reassign to...</option>
                {0}
            </select>
            <input type="button" onclick="reassignToUser()" value=" Confirm Reassignment " disabled="true"/>'''.format(''.join(users))
        else :
            assignmentOptn = ''
            self.htmlNav[2] = '''\
            <li class="navtab">
...
#        if quota - total <= 10:
    ...
    def _get_totalDrafts(self):
        ...
        result = res.dictresult()
        authinst = result[0]['institutionid']
        countsql = "SELECT COUNT(editingstore) as total FROM editingstore_linkrecinst WHERE institutionid = %s" % authinst
        res2 = editStore._query(countsql)
    ...
    def _walk_store(self, store, type='checkbox'):
        ...
        if len(result) > 1:
            ...
            institutionid = result[0]['institutionid']
        sqlQ = "SELECT DISTINCT editingstore FROM editingstore_linkrecinst WHERE institutionid='%s' ORDER BY editingstore" % institutionid
        res = authStore._query(sqlQ)
        ...
        for name in self._get_usernamesByInst(institutionid):
            names.append(name)
            if name == session.user.username or session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
                disabled = ''
            else :
                disabled = 'disabled="disabled"'
            userFiles = ['<li title="{0}" id="{0}"><span>{0}</span>'.format(name), 
                         '<ul class="hierarchy">'] 
            for s in records:
                ...
                if recid[recid.rfind('-')+1:] == name:
                    try :
                        ...
                        displayId = recid
                    userFiles.extend(['<li>'
                                       ,'<span class="fileops"><input type="%s" name="recid" value="%s" %s/></span>' % (type, recid, disabled)
        ...
        sqlQ = "SELECT DISTINCT editingstore FROM editingstore_linkrecinst WHERE institutionid='%s'" % institutionid
        res = authStore._query(sqlQ)
        result = res.dictresult() 
        if total < len(result):
            out.extend(['<li title=deletedUsers><span>Deleted Users</span>', '<ul class="hierarchy">']) 
            if session.user.has_flag(session, 'info:srw/operation/1/create', 'hubAuthStore'):
                disabled = ''
            else :
                disabled = 'disabled="disabled"'
    ...
    def handle(self, req):
        ...
        if path == 'users.html':
            if (operation):
                ...
                content = self.show_userMenu()
            page = multiReplace(read_file(templatePath), self.globalReplacements)
            page = multiReplace(page, {'%TITLE%': ' :: '.join(self.htmlTitle),
                     '%NAVBAR%': ' '.join(self.htmlNav),
                     '%CONTENT%': content
                     })
            # send the display
            ...
            if (operation == 'add'):  
                ...
                self.send_xml('''\
<wrap><recid>{0}</recid><valid>{1}</valid><username>{2}</username></wrap>'''.format(content, valid, session.user.username), req)
...
def build_architecture(data=None):
    global session, serv, db, editStore, authStore, formTxr, tocTxr, xmlp, assignDataIdFlow, ppFlow, fullTxr, fullSplitTxr
    #Discover objects
    session = Session()
...
def handler(req):
    ...
    try :
        ...
        # handle request
        try:
            #cProfile.runctx('hubEditingHandler.handle(req)', globals=globals(), locals=locals(), filename='../logs/menuloadstat')
        ...
        req.log_error('*** {0}: {1} ; {2}'.format(excName, excArgs, '; '.join(excTb)))
        return apache.HTTP_INTERNAL_SERVER_ERROR
    else :
        return apache.OK
    #- end handler()
    
    #cProfile.runctx('searchHandler.handle(req)', globals=globals(), locals=locals(), filename='../logs/wwwsearchstat')
