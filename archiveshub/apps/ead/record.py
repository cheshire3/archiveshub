...
class EADRecordWsgiApplication(EADWsgiApplication):
    ...
    def html_generate_page(self, rec, pagenum):
        """Generate a page and store a cache version"""
        recid = rec.id
        # Transform the Record
        doc_uc = self.html_transform_record(rec)
        ...
        if len(divs) > 0:
            tocdiv = divs[0]
            divs = divs[1:]
            pages, anchorPageHash = self.html_pages(divs)
    ...
    def html_generate_toc(self, rec):
        """Generate the TOC and store a cache version"""
        recid = rec.id
        # Transform the Record
        doc_uc = self.html_transform_record(rec)
        # Parse HTML fragments
    ...
    def html_pages(self, divs):
        """Get a list of div elements and returns:
        ...
        # Read final page
        pages.append(self._readBuffer(pageBuffer))
        return pages, anchorPageHash

    def html_transform_record(self, rec):
        doc_uc = self._transformRecord(rec, 'htmlFullTxr')
    ...
    def html(self, rec, form):
        ...
        if os.path.exists(path):
            ...
            try:
                ...
                # We try to generate it in case something prevent the generation
                page = self.html_generate_page(rec, pagenum)
                if not page:
            ...
            try:
                ...
                toc = self.html_generate_toc(rec)
