...
<response>
    ...
    def display_full(self, rec, paramDict):
        ...
        pageSizeBound = maximum_page_size - 1
        pages = []
        anchorPageHash = {}
        for div in divs:
        ...
        if toc is not None:
            try:
                for k, v in anchorPageHash.iteritems():
                    toc = toc.replace(
                        u'PAGE#%s"' % k,
                        u'%s/%s-p%d.shtml#%s"' % (cache_url, recid, v, k)
                    )
            ...
            toc = multiReplace(
                toc,
                {u'SCRIPT': self.script,
                 u'PAGE#': u'%s/%s-p1.shtml#' % (cache_url, recid)
                 }
            )
            toc = '\n'.join([
                '<div id="toc">',
                toc_scripts.replace('%RECID%', recid).replace('RECID', recid),
                toc.encode('ascii', 'xmlcharrefreplace'),
                '</div><!-- /toc -->'
            ])
            write_file(os.path.join(toc_cache_path, recid + '.inc'), toc)
