...
<xsl:stylesheet
    ...
    <xsl:template name="count-subcomponents">
        ...
        <!-- <p> <xsl:element name="a"> <xsl:attribute name="href"> <xsl:text>SCRIPT?operation=search&amp;withinCollection=RECID&amp;cqlquery=QSTRING</xsl:text> 
            <xsl:value-of select="./@target"/> </xsl:attribute> <xsl:attribute name="target"> 
            <xsl:text>_top</xsl:text> </xsl:attribute> <xsl:attribute name="title"> <xsl:text>Repeat 
            your search within this collection only.</xsl:text> </xsl:attribute> <xsl:text>Search 
            within this collection</xsl:text> </xsl:element> </p> -->
    </xsl:template>
