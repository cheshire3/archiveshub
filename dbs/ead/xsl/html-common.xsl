...
<xsl:stylesheet
    ...
    <xsl:template match="did" mode="didtable">
        ...
        <xsl:param name="component-identifier">
            <xsl:apply-templates select="/*/*/did" mode="getid" />
        </xsl:param>
        
         <xsl:param name="parent-identifier-comp">
            <xsl:choose>
        ...
        <xsl:param name="parent-identifier">
            <xsl:apply-templates select="/ead/archdesc/did" mode="getid"/>
        </xsl:param>
            ...
            <tr>
                ...
                <td>
                    <xsl:variable name="path">
	                    ...
	                    <xsl:choose>
	                        <xsl:when test="parent::archdesc and /ead">
                                <xsl:apply-templates select="." mode="getid"/>
                            </xsl:when>
                            <xsl:when test="/ead">
                               <xsl:call-template name="normalize" mode="getid">
                                    <xsl:with-param name="text">
                                    <xsl:value-of
                                        select="concat($parent-identifier, '/', unitid)" />
                                    </xsl:with-param>
                                </xsl:call-template>
                            </xsl:when>
	                        <xsl:otherwise>
                               <xsl:call-template name="normalize" mode="getid">
                                    <xsl:with-param name="text">
                                    <xsl:value-of
                                        select="concat($parent-identifier-comp, '/', $component-identifier)" />
                                    </xsl:with-param>
                                </xsl:call-template>
                            </xsl:otherwise>
	                    </xsl:choose>
                ...
                <!--xsl:value-of select="/ead/archdesc/did/unittitle"/--> <a href="/data/{$path}"><xsl:value-of select='$path'/></a>
    ...
    <xsl:template match="did" mode="getid">
        <xsl:choose>
            ...
            <xsl:when test="unitid[@type='persistent']">
                <xsl:apply-templates select="unitid[@type='persistent'][1]" mode="getid" />
            </xsl:when>
            <xsl:when test="unitid[@label]">
                ...
                <xsl:choose>
                    <xsl:when
                        ...
                        <xsl:apply-templates
                            select="unitid[not(starts-with(@label, 'Former')) and not(starts-with(@label, 'former'))][@countrycode and @repositorycode and @identifier][1]" mode="getid" />
                    </xsl:when>
                    <xsl:when
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when
                        ...
                        <!-- when all 3 attributes are present -->
                        <xsl:apply-templates
                            select="unitid[@countrycode and @repositorycode and @identifier][1]" mode="getid" />
                    </xsl:when>
                    <xsl:when test="unitid[@countrycode and @repositorycode]">
                        <!-- when code attributes are present -->
                        <xsl:apply-templates
                            select="unitid[@countrycode and @repositorycode][1]" mode="getid" />
                    </xsl:when>
                    <xsl:otherwise>
                        <!-- take the first that's there -->
                        <xsl:apply-templates select="unitid[1]" mode="getid" />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="unitid" mode="getid" >
    ...
    <xsl:template name="normalize" mode="getid">
        <!--
